...
class TheaterChase3(ColorCycleTemplate):
    def update(self, strip, num_led, num_steps_per_cycle, current_step,
               current_cycle):
        start_index = current_step % 7
        color_index = strip.combine_color(255,0,0)
        for pixel in range(num_led):
            if ((pixel+start_index) % 7 == 0) or ((pixel+start_index) % 7 == 1):
                strip.set_pixel_rgb(pixel, 0)
            else: strip.set_pixel_rgb(pixel, color_index)
...
class TheaterChase4(ColorCycleTemplate):
    def update(self, strip, num_led, num_steps_per_cycle, current_step,
               current_cycle):
        start_index = current_step % 7
        color_index = strip.combine_color(255,40,40)
        for pixel in range(num_led):
            if ((pixel+start_index) % 7 == 0) or ((pixel+start_index) % 7 == 1):
                strip.set_pixel_rgb(pixel, 0)
            else: strip.set_pixel_rgb(pixel, color_index)
...
class TheaterChase5(ColorCycleTemplate):
    def update(self, strip, num_led, num_steps_per_cycle, current_step,
               current_cycle):
        start_index = current_step % 7
        color_index = strip.combine_color(40,0,255)
        for pixel in range(num_led):
            if ((pixel+start_index) % 7 == 0) or ((pixel+start_index) % 7 == 1):
                strip.set_pixel_rgb(pixel, 0)
            else: strip.set_pixel_rgb(pixel, color_index)
...
class PinkChase(ColorCycleTemplate):
    def update(self, strip, num_led, num_steps_per_cycle, current_step,
               current_cycle):
        start_index = current_step % 7
        color_index = strip.combine_color(255,40,40)
...
class PurpleChase(ColorCycleTemplate):
    def update(self, strip, num_led, num_steps_per_cycle, current_step,
               current_cycle):
        start_index = current_step % 7
...
class OrangeChase(ColorCycleTemplate):
    def update(self, strip, num_led, num_steps_per_cycle, current_step,
               current_cycle):
        start_index = current_step % 7
        color_index = strip.combine_color(255,0,165)
        for pixel in range(num_led):
            if ((pixel+start_index) % 7 == 0) or ((pixel+start_index) % 7 == 1):
                strip.set_pixel_rgb(pixel, 0)
            else: strip.set_pixel_rgb(pixel, color_index)
...
class Solid(ColorCycleTemplate):
    ...
    def update(self, strip, num_led, num_steps_per_cycle, current_step,
        ...
        for led in range(0, num_led):
            strip.set_pixel_rgb(led,stripcolour,80) # Paint 5% white  
        return 1
